...
class TagsController extends DefaultController {
    ...
    public function view() {
        ...
        //查询操作
        $sql = "SELECT * FROM `".$tagsDB->getTableName()."` WHERE {$whereStr} ORDER BY {$orderStr}";
        $data = $tagsDB->getData($sql, $whereArr, 20);
        
        $pageStr = $tagsDB->getPageStr();
        //读取外键数据
        
        //模版显示
        $this->setView('pageStr', $pageStr);
        $this->setView('data', $data);
        $this->display('Tags.html');
    }
    ...
    public function get() {
        ...
        $sql = "SELECT * FROM `".$tagsDB->getTableName()."` WHERE {$whereStr}";
        $row = $tagsDB->getRow($sql, $whereArr);
        
        $row ? Message::showSucc('获取单条标签数据', $row) : Message::showError('没有获取到指定数据，可能已被删除', $row);
    }
    ...
    public function create() {
        ...
        $rs = $tagsDB->insert($insertArr);
        Log::write($this->adminUserName, $this->ip, $tagsDB->insertId(), self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('添加成功') : Message::showError('添加失败：' . implode(" ", $tagsDB->getErrorInfo()));
    }
    ...
    public function update() {
        ...
        $sql = "SELECT * FROM `".$tagsDB->getTableName()."` WHERE `id` = ?";
        $row = $tagsDB->getRow($sql, array_values($whereArr));
        ...
        $rs = $tagsDB->update($changeNewRow, $whereArr);
        //获取被修改数据写入日志
        ...
        //更新提示
        $rs ? Message::showSucc('更新成功', $changeNewRow) : Message::showError('更新失败：' . implode(" ", $tagsDB->getErrorInfo()));
    }
    ...
    public function delete() {
        ...
        $rs = $tagsDB->delete($whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs, Log::arrayToLog($row));
        $rs ? Message::showSucc('删除成功') : Message::showError('删除失败：' . implode(" ", $tagsDB->getErrorInfo()));
    }
    ...
    public function logicDelete() {
        ...
        $rs = $tagsDB->update($updateArr, $whereArr);
    ...
    public function logicResume() {
        ...
        $rs = $tagsDB->update($updateArr, $whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('恢复成功') : Message::showError('恢复失败：' . implode(" ", $tagsDB->getErrorInfo()));
    }
}
?>
