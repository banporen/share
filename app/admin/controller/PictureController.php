...
class PictureController extends DefaultController {
    ...
    public function view() {
        ...
        //查询操作
        $sql = "SELECT * FROM `".$pictureDB->getTableName()."` WHERE {$whereStr} ORDER BY {$orderStr}";
        $data = $pictureDB->getData($sql, $whereArr, 20);
    ...
    public function get() {
        ...
        $sql = "SELECT * FROM `".$pictureDB->getTableName()."` WHERE {$whereStr}";
        $row = $pictureDB->getRow($sql, $whereArr);
    ...
    public function create() {
        ...
        $rs = $pictureDB->insert($insertArr);
        Log::write($this->adminUserName, $this->ip, $pictureDB->insertId(), self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('添加成功') : Message::showError('添加失败：' . implode(" ", $pictureDB->getErrorInfo()));
    }
    ...
    public function update() {
        ...
        $sql = "SELECT * FROM `".$pictureDB->getTableName()."` WHERE `id` = ?";
        $row = $pictureDB->getRow($sql, array_values($whereArr));
        ...
        $rs = $pictureDB->update($changeNewRow, $whereArr);
        //获取被修改数据写入日志
        ...
        //更新提示
        $rs ? Message::showSucc('更新成功', $changeNewRow) : Message::showError('更新失败：' . implode(" ", $pictureDB->getErrorInfo()));
    }
    ...
    public function delete() {
        ...
        $rs = $pictureDB->delete($whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs, Log::arrayToLog($row));
        $rs ? Message::showSucc('删除成功') : Message::showError('删除失败：' . implode(" ", $pictureDB->getErrorInfo()));
    }
    ...
    public function logicDelete() {
        ...
        $rs = $pictureDB->update($updateArr, $whereArr);
    ...
    public function logicResume() {
        ...
        $rs = $pictureDB->update($updateArr, $whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('恢复成功') : Message::showError('恢复失败：' . implode(" ", $pictureDB->getErrorInfo()));
    }
}
?>
