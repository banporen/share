...
class CollectController extends DefaultController {
    ...
    public function view() {
        ...
        //查询操作
        $sql = "SELECT * FROM `".$collectDB->getTableName()."` WHERE {$whereStr} ORDER BY {$orderStr}";
        $data = $collectDB->getData($sql, $whereArr, 20);
        
        $pageStr = $collectDB->getPageStr();
        //读取外键数据
    ...
    public function get() {
        ...
        $sql = "SELECT * FROM `".$collectDB->getTableName()."` WHERE {$whereStr}";
        $row = $collectDB->getRow($sql, $whereArr);
        
        $row ? Message::showSucc('获取单条收藏数据', $row) : Message::showError('没有获取到指定数据，可能已被删除', $row);
    }
    ...
    public function create() {
        ...
        $rs = $collectDB->insert($insertArr);
        Log::write($this->adminUserName, $this->ip, $collectDB->insertId(), self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('添加成功') : Message::showError('添加失败：' . implode(" ", $collectDB->getErrorInfo()));
    }
    ...
    public function update() {
        ...
        $sql = "SELECT * FROM `".$collectDB->getTableName()."` WHERE `id` = ?";
        $row = $collectDB->getRow($sql, array_values($whereArr));
        ...
        $rs = $collectDB->update($changeNewRow, $whereArr);
        //获取被修改数据写入日志
        ...
        //更新提示
        $rs ? Message::showSucc('更新成功', $changeNewRow) : Message::showError('更新失败：' . implode(" ", $collectDB->getErrorInfo()));
    }
    ...
    public function delete() {
        ...
        $rs = $collectDB->delete($whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs, Log::arrayToLog($row));
        $rs ? Message::showSucc('删除成功') : Message::showError('删除失败：' . implode(" ", $collectDB->getErrorInfo()));
    }
    ...
    public function logicDelete() {
        ...
        $rs = $collectDB->update($updateArr, $whereArr);
    ...
    public function logicResume() {
        ...
        $rs = $collectDB->update($updateArr, $whereArr);
        Log::write($this->adminUserName, $this->ip, $_GET['id'], self::$controller . "__" . self::$action, $rs);
        $rs ? Message::showSucc('恢复成功') : Message::showError('恢复失败：' . implode(" ", $collectDB->getErrorInfo()));
    }
}
?>
